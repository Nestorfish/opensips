...
int reload_address_table(struct pm_part_struct *part_struct)
{
	...
	for (i = 0; i < RES_ROW_N(res); i++) {
		...
		if (!new_netmask_table[mask].hash_table) {
			new_netmask_table[mask].hash_table = hash_create();
			if (!new_netmask_table[mask].hash_table) goto error;
		}

		subnet = mk_net_bitlen(ip_addr, mask);
		if (hash_insert(new_netmask_table[mask].hash_table, subnet, group, port, proto,
...
int check_addr_6(struct sip_msg* msg,
	...
	netmask_node = (*part_struct->netmask_table) + bitlen;
	while (1) {
		if (netmask_node->hash_table) {
			ret = hash_match(msg, netmask_node->hash_table, group, &ip_match, port,
					proto, pattern, info);
			if (ret > 0) break;
		}
...
int check_src_addr_3(struct sip_msg *msg,
	...
	LM_DBG("Looking for : <%d, %s, %d, %d> in tables\n",
				group, ip_addr2a(&msg->rcv.src_ip),
				msg->rcv.src_port,
				msg->rcv.proto);
	if (pattern) {
		if (fixup_get_svalue(msg, (gparam_p)pattern, &pattern_s) < 0) {
	...
	while (1) {
		if (netmask_node->hash_table) {
			ret = hash_match(msg, netmask_node->hash_table, group,
				&ip_match,
				msg->rcv.src_port,
				msg->rcv.proto,
				pattern,
...
int get_source_group(struct sip_msg* msg, char *arg) {
	...
	netmask_node = (*ps->netmask_table) + bitlen;
	while (1) {
		if (netmask_node->hash_table) {
